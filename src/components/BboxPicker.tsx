...
export default function BboxPicker({ bbox, onChange, tiles }: { bbox: Bbox; onChange: (b: Bbox) => void; tiles: string }) {
  ...
  React.useEffect(() => {
    ...
    map.on('load', () => {
      ...
      drawFromState();
      map.resize();
      // extra resizes to handle late layout
  ...
  // Update tiles when prop changes
  React.useEffect(() => {
    const map = mapRef.current; if (!map || !styleReadyRef.current) return;
    try {
      if (map.getLayer('tiles-layer')) map.removeLayer('tiles-layer');
      if (map.getSource('tiles')) map.removeSource('tiles');
      if (tiles) {
        map.addSource('tiles', { type: 'raster', tiles: [tiles], tileSize: 256 } as any);
        // Place below bbox lines so selection remains visible
        map.addLayer({ id: 'tiles-layer', type: 'raster', source: 'tiles' } as any, 'bbox-line');
      }
      map.resize();
    } catch {}
  }, [tiles]);
  ...
  return (
    <div className="relative w-full h-56 rounded-2xl overflow-hidden border bg-white dark:bg-neutral-950">
